...
这是一个完全托管在 GitHub Actions 上的 Python 自动化项目。它每天收集近 24 小时的 AI、计算机、编程、开源与开发者工具动态，使用 DeepSeek 生成中文摘要，并通过 Resend 发送一封适配 Outlook 的 HTML 邮件。
...
4. 向 DeepSeek 提供标题、来源、时间与来源摘录；模型只能据此编写中文标题、2–4 句摘要和标签。原始链接与来源不会交给模型改写。
...
- **DeepSeek 摘要失败**：日志会记录原因；邮件会发送可核验的原始来源兜底内容。确认 API Key、模型名和账户余额。
